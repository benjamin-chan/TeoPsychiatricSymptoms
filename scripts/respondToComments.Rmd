...
dfTemp <-
  ...
  mutate(marital = factor(marital,
                          labels = c("Single, never been married",
                                     ...
                                     "Separated"))) %>% 
  mutate(marital = factor(marital,
                          labels = c("Single/Divorce/Widowed/Separated",
                                     "Married or living as married",
                                     "Married or living as married",
                                     "Single/Divorce/Widowed/Separated",
                                     "Single/Divorce/Widowed/Separated",
                                     "Single/Divorce/Widowed/Separated"))) %>% 
  mutate(marital = factor(marital))
align <- "llcccccc"
...
dfTemp %>% freq2Group("fb_freq", "marital") %>% kable(align = align, digits = digits)
chisq.test(dfTemp$marital, dfTemp$fb_freq)
```
...
The difference was only `r sprintf("%.01f years", dfTemp %>% t.test(age ~ fb_freq, data = .) %>% .[["estimate"]] %>% as.vector() %>% diff() %>% abs())`.
