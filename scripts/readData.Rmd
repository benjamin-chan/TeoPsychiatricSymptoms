...
df0 <-
  ...
  mutate(raceNonHispWhite = case_when(race == "White" & hispanic == "No" ~ "Non-Hispanic White",
                            ...
                            factor(levels = c("Not non-Hispanic White",
                                              "Non-Hispanic White"))) %>%
...
```{r}
df0 %>% pull(age) %>% summary()
df0 %>% group_by(gender) %>% summarize(n = n()) %>% kable()
df0 %>% group_by(fct_explicit_na(race)) %>% summarize(n = n()) %>% kable()
df0 %>% group_by(fct_explicit_na(hispanic)) %>% summarize(n = n()) %>% kable()
df0 %>% group_by(fct_explicit_na(marital)) %>% summarize(n = n()) %>% kable()
df0 %>% group_by(fct_explicit_na(marital2)) %>% summarize(n = n()) %>% kable()
```
