...
df0 <- 
  ...
  mutate(fmss_r1 = factor(fmss_r1, 
         ...
         fmss_r3 = factor(fmss_r3, 
                          levels = 0:4, 
                          labels = c("Not at all", "A little", "Somewhat", "Quite a bit", "A lot")),
...
df0 <- 
  df0 %>% 
  mutate(fmss_r1 = factor(fmss_r1,
                          levels = levels(df0$fmss_r1),
                          labels = c(levels(df0$fmss_r1)[1:3], rep("Very or Extremely", 2))),
         fmss_r3 = factor(fmss_r3,
                          levels = levels(df0$fmss_r3),
                          labels = c(levels(df0$fmss_r3)[1:3], rep("Quite a bit or A lot", 2))),
         fmss_r11 = factor(fmss_r11,
                           levels = levels(df0$fmss_r11),
                           labels = c(levels(df0$fmss_r11)[1:3], rep("Very or Extremely", 2)))) %>% 
